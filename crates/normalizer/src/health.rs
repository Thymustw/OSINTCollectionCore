//! normalizer 的 `/health` `/ready` `/metrics`。
...
pub async fn serve(
    ...
    let addr: SocketAddr = bind.parse().map_err(|err| NormalizerError::Configuration {
        message: format!("normalizer.bind `{bind}` 不是合法位址：{err}"),
    })?;
    let app = router(HealthState { metrics, store });
    let listener = TcpListener::bind(addr)
        .await
        .map_err(|err| NormalizerError::Configuration {
            message: format!("綁定 {addr} 失敗：{err}。請改 config [normalizer].bind 或釋放該埠"),
        })?;
    tracing::info!(%addr, "normalizer health 開始聽");
