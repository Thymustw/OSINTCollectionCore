...
//! candidate 寫回去。
//!
//! # [`ResolverService::resolve_entity`] 目前聚合五種掃描方法
//!
...
//! 生產呼叫端。性質不同，保持獨立。
//!
//! `account_handle`／`email`／`external_id` 仍未做：同一個 UNIQUE 限制讓
//! 「查自己 identifier 的目前 owner」永遠是自己，需要各自的跨欄位設計，
//! 不是抄舊版 `check_domain` 的形狀就能做。SPEC §6 也禁止只因同 username
//! 就判定同一真實人物。
//!
...
pub use identifier_methods::{check_alias, check_domain};
