//! `job.dispatched` → STIX 2.1 bundle 匯入 Entity／Relationship／Provenance。
//!
//! # 這一步只做 `stix_import`
//!
//! `job_type` 不是 [`JOB_TYPE_IMPORT`] 的事件（含 `stix_export`、`graph_rebuild`）
//! 一律 [`JobDispatchOutcome::Ignored`] 並提交 offset。匯出留給 Step 4。
//!
//! # PostgreSQL 是 truth
//!
...
pub use service::{
    ACTION_STIX_IMPORTED, JOB_TYPE_IMPORT, JobDispatchOutcome, PROCESSOR, STIX_DEFAULT_CONFIDENCE,
    StixWorker, StixWorkerOptions,
};
